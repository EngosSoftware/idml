...
fn _0003() {
  ...
  let input = "\t";
  assert_eq!("unexpected character: '\t' 0x09 at row 1 and column 1", tokenize(input).unwrap_err().to_string());
}
...
fn _0006() {
  ...
  let input = ".\n";
  assert_eq!("unexpected character: '\n' 0x0A at row 1 and column 2", tokenize(input).unwrap_err().to_string());
}
...
fn _0007() {
  ...
  let input = ".\r";
  assert_eq!("unexpected character: '\r' 0x0D at row 1 and column 2", tokenize(input).unwrap_err().to_string());
}
